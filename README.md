...
│
├── tests/                                  # Google Test unit tests (21 files, 198 tests)
│   ├── test_cap_event.cpp
...
| `MAS_ENABLE_ZMQ` | `ON` | Build the ZeroMQ agent runtime (`mas_transport`, `mas_worker`, `mas_coordinator`) and its 9 transport tests. The agent layer's 58 tests run on `FakeTransport` in every store build, ZMQ or not. |
...
had: **187 tests green**. With `MAS_BENCH_ONLY=ON` **no tests are built at
...
The project has **198 C++ unit tests** across 21 Google Test files — 187 in the
...
```bash
cd build && ctest --output-on-failure           # 187 C++ tests in the default build; the 11-case GPU/CPU differential is compiled only with -DMAS_ENABLE_CUDA=ON (and skips without a device)
...
`test_bench_cpu_parity.cpp` — so on a fresh clone **187 tests green** means 185
