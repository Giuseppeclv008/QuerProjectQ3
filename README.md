...
| `MAS_ENABLE_ZMQ` | `ON` | Build the ZeroMQ agent runtime (`mas_transport`, `mas_worker`, `mas_coordinator`) and its 9 transport tests. The agent layer's 63 tests run on `FakeTransport` in every store build, ZMQ or not. |
...
cd python && ../.venv/bin/python -m pytest -q    # 307 Python tests (see the three gates below)
```
...
`ANTHROPIC_API_KEY` and skip without it, because they send this project's plan
schemas to the real Anthropic structured-output API and spend from the
account — see
`python/tests/test_anthropic_schema_live.py`, which is the only test in the
suite that leaves the machine.

So a fresh clone shows 9 skips; a machine with the pool extracted but no store
shows 7; and adding a key removes 2 more.
