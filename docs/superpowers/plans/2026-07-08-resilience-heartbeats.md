...
- **Execution-time correction 2 (2026-07-09, Task 7):** the chaos run's coordinator-death direction exposed a latent Plan 3 defect this plan inherited: `ZmqPushSink` coupled `ZMQ_LINGER` to `send_timeout_ms` (60 s), so an orphaned worker returned from `run()` on schedule (~61 s) but the process then sat out the full linger in `zmq_ctx_term` draining ~61 queued heartbeats to a peer that never existed — total ~121 s versus the spec's ≤ ~60 s exit promise. Fix (controller-adjudicated): `ZmqPushSink` gains a separate `linger_ms` parameter (default sentinel preserves the old coupling for all existing callers); `worker_main` passes `linger_ms=0` on its results and heartbeat sinks, which is protocol-safe because the coordinator holds every result before it sends STOP and heartbeats are fire-and-forget. Regression-tested by a teardown-wall-clock transport test plus the re-run chaos direction 2. The unit suite structurally could not have caught this (fake transports have no linger); only the live E2E could — which is why Task 7 exists.
