...
std::optional<WorkResult> decode_result(const Message& m) {
    ...
    try {
        ...
        // seven orders of magnitude. The failure channel is two codes wide:
        // -1 (input unreadable, deterministic) and -2 (the store failed with
        // rows already written, so the item is retryable). Anything below -2
        // is malformed. `seconds` has the same gap ("inf" and "nan"
        // stod-parse with full consumption): require finite, non-negative,
        // and under 10^9 (~31 years).
        if (events < -2 || events > 1000000000000LL) return std::nullopt;
