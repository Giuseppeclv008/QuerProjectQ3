...
// one function, not three copies. It was three, and the GPU copy kept a
// truncating cast after both CPU copies were fixed — a wrapped delta
// fabricates small (or <= 1, aggregated-clearing) values on exactly the input
// class CapEvent.hpp documents. A single __host__ __device__ definition makes
// that divergence impossible and lets the policy be unit-tested on a machine
// with no CUDA toolchain.
