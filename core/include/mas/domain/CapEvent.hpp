...
// Maximum integer value (2^53) that can be exactly represented by an IEEE-754
// double
inline constexpr double MAX_EXACT_DOUBLE_INT = 9007199254740992.0;

// Maximum/minimum float values to prevent double -> float overflow
inline constexpr double MAX_FLOAT_VAL = std::numeric_limits<float>::max();
inline constexpr double MIN_FLOAT_VAL = std::numeric_limits<float>::lowest();
...
//   status 2,  torque == 0  -> "No Load" cycle: counter advances, no cap
//   applied status 65, torque > 0   -> Bad Closure, rejected status 9,  torque
//   > 0   -> No InTorque, rejected status 4,  torque >= 0  -> No Closure, not
//   rejected
// `% 2 == 1` would be wrong for a negative status: C++ truncates toward zero,
// so -65 % 2 is -1 and a rejected closure would read as clean. The PLC has
// never emitted a negative status in this pool, but the guard is free and the
...
inline bool is_reject(double status) {
  ...
  if (!(status >= -MAX_EXACT_DOUBLE_INT && status <= MAX_EXACT_DOUBLE_INT))
...
inline bool is_valid_count(double c) {
  return c >= -MAX_EXACT_DOUBLE_INT && c <= MAX_EXACT_DOUBLE_INT;
}
...
inline bool is_valid_real(double v) {
  return v >= MIN_FLOAT_VAL && v <= MAX_FLOAT_VAL;
